...
class Button:
    def __init__(
        ...
        self.button, self.button_text_surface, self.button_text_rect = (
            self._create_button()
        )
    ...
    def _create_button(self):
        ...
        pygame.draw.circle(
            button_image, self.color, (self.radius, self.radius), self.radius
        )

        try:
            icon_image = pygame.transform.scale(
                pygame.image.load(
                    os.path.join("..", "data", self.icon_path)
                ).convert_alpha(),
        ...
        button_text_surface, button_text_rect = self._render_text(
            self.text,
            self.font,
            (0, 0, 0),
            pos=(self.x, self.y + self.radius + self.text_offset_y),
        )
...
class Menu:
    def __init__(self, screen):
        ...
        self.screen = screen
        self.cloud_image, self.cloud_image_rect = self.load_image(
            "cloud.png", colorkey=-1, scale=(200, 150), pos=(30, 10)
        )
        self.title_surface, self.title_rect = self.render_text(
            "Jump: Приключение прыгуна", self.font, (0, 0, 0)
        )
        self.menu_music_is_playing = [
            item
            for item in self.db_cursor.execute(
                "SELECT music_is_playing FROM game_settings"
            )
        ][-1][-1]
    ...
    def load_image(self, name, colorkey=None, scale=None, pos=None):
        ...
        if pos:
            image_rect.x, image_rect.y = pos[0], pos[1]
    ...
    def open_level_list(self):
        self.menu_music.stop()
        if (
            not self.showing_level_list
        ):  # Проверяем, отображается ли уже окно выбора скина
    ...
    def build_level_list(self):
        self.lbc = list(
            item
            for item in self.db_cursor.execute("SELECT * FROM GAME_PROCESS")
        )[0]
        self.first_btn = PygameWidgetsButton(
            self.screen_level_list,
            100,
            100,
            800,
            50,
            inactiveColour=self.levels_buttons_colors[self.lbc[0]][0],
            hoverColour=self.levels_buttons_colors[self.lbc[0]][1],
            pressedColour=self.levels_buttons_colors[self.lbc[0]][2],
        )
        self.second_btn = PygameWidgetsButton(
            ...
            800,
            50,
            inactiveColour=self.levels_buttons_colors[self.lbc[1]][0],
            hoverColour=self.levels_buttons_colors[self.lbc[1]][1],
            pressedColour=self.levels_buttons_colors[self.lbc[1]][2],
        )
        self.third_btn = PygameWidgetsButton(
            ...
            800,
            50,
            inactiveColour=self.levels_buttons_colors[self.lbc[2]][0],
            hoverColour=self.levels_buttons_colors[self.lbc[2]][1],
            pressedColour=self.levels_buttons_colors[self.lbc[2]][2],
        )
        self.fourth_btn = PygameWidgetsButton(
            ...
            800,
            50,
            inactiveColour=self.levels_buttons_colors[self.lbc[3]][0],
            hoverColour=self.levels_buttons_colors[self.lbc[3]][1],
            pressedColour=self.levels_buttons_colors[self.lbc[3]][2],
        )
        self.btn_of_infinity = PygameWidgetsButton(
            ...
            800,
            50,
            inactiveColour=self.levels_buttons_colors[self.lbc[4]][0],
            hoverColour=self.levels_buttons_colors[self.lbc[4]][1],
            pressedColour=self.levels_buttons_colors[self.lbc[4]][2],
        )
        self.leave_from_ll = PygameWidgetsButton(
            self.screen_level_list, 100, 490, 800, 100
        )
        self.first_btn.font = self.second_btn.font = self.third_btn.font = (
            self.fourth_btn.font
        ) = self.btn_of_infinity.font = self.leave_from_ll.font = self.font
        ...
        (
            ...
            if self.levels_buttons_colors[self.lbc[0]][-1]
            else self.first_btn.disable()
        )
        (
            self.second_btn.enable()
            if self.levels_buttons_colors[self.lbc[1]][-1]
            else self.second_btn.disable()
        )
        (
            self.third_btn.enable()
            if self.levels_buttons_colors[self.lbc[2]][-1]
            else self.third_btn.disable()
        )
        (
            self.fourth_btn.enable()
            if self.levels_buttons_colors[self.lbc[3]][-1]
            else self.fourth_btn.disable()
        )
        (
            self.btn_of_infinity.enable()
            if self.levels_buttons_colors[self.lbc[4]][-1]
            else self.btn_of_infinity.disable()
        )
    ...
    def select_skin(self):
        if (
            not self.showing_skin_selector
        ):  # Проверяем, отображается ли уже окно выбора скина
    ...
    def create_skin_selector_buttons(self):
        ...
        self.current_character_index = (
            self.character_names.index(self.selected_character)
            if self.selected_character in self.character_names
            else 0
        )

        self.skin_selector_buttons = [
            PygameWidgetsButton(
                self.skin_selector_screen,
                50,
                100,
                button_width,
                button_height,
            ...
            PygameWidgetsButton(
                ...
                text="Меню",
                font=font,
                onClick=self.hide_skin_selector,
            ),
        ]
    ...
    def draw_skin_selector(self):
        if self.skin_selector_buttons and self.showing_skin_selector:
            ...
            try:
                character_image = pygame.transform.scale(
                    pygame.image.load(
                        os.path.join(
                            "..",
                            "data",
                            self.character_names[self.current_character_index],
                        )
                    ).convert_alpha(),
                    (400, 400),
                )
                ...
                print(
                    f"Ошибка загрузки картинки персонажа: {
    ...
    def save_skin(self):
        self.selected_character = self.character_names[
            self.current_character_index
        ]
    ...
    def save_selected_character(self, character_name):
        if self.db_cursor:
            try:
                self.db_cursor.execute(
                    "UPDATE game_settings SET setting_value = ? WHERE setting_name = ?",
                    (character_name, "selected_character"),
                )
                if self.db_cursor.rowcount == 0:
                    self.db_cursor.execute(
                        "INSERT INTO game_settings (setting_name, setting_value) VALUES (?, ?)",
    ...
    def account_manage(self):
        if not self.showing_account_management:
            ...
            self.account_text = self.font.render(
                f"Наиграно времени: {round(list(self.db_cursor.execute(
                'SELECT TIME_PLAYED FROM GAME_SETTINGS'),)[0][0], 1)} ч.",
                True,
                "BLACK",
            )
            self.account_screen.blit(self.account_text, (100, 150))
            self.account_text = self.font.render(
                f"Собрано монет в INFINITY MODE: {list(
                self.db_cursor.execute('SELECT SCORE FROM GAME_PROCESS'))[0][0]}",
                True,
                "BLACK",
            )
    ...
    def play_music_menu(self):
        ...
        self.db_cursor.execute(
            f"UPDATE game_settings SET music_is_playing={
            self.menu_music_is_playing}"
        )
        self.db_connection.commit()
